...
export const hasAdminAccess = (user: User): boolean => {
  return checkPermission(user, Permission.ADMIN_ACCESS) || user.role === UserRole.ADMIN;
};
...
export const canManageApiKeys = (user: User): boolean => {
  return checkPermission(user, Permission.API_KEY_MANAGE) || hasAdminAccess(user);
};
...
export const canManageSettings = (user: User): boolean => {
  return checkPermission(user, Permission.SETTINGS_MANAGE) || hasAdminAccess(user);
};
...
export const canExportData = (user: User): boolean => {
  return checkPermission(user, Permission.DATA_EXPORT) || hasAdminAccess(user);
};
...
export const canViewAuditLogs = (user: User): boolean => {
  return checkPermission(user, Permission.AUDIT_VIEW) || hasAdminAccess(user);
};
...
export const canDelete = (user: User): boolean => {
  return checkPermission(user, Permission.DELETE) || hasAdminAccess(user);
};
...
export const canWrite = (user: User): boolean => {
  return checkPermission(user, Permission.WRITE) || hasAdminAccess(user);
};
