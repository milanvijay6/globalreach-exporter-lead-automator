...
router.get('/wechat', async (req, res) => {
  try {
    ...
    // Ensure params are strings to prevent type juggling attacks
    const strToken = String(webhookToken);
    const strTimestamp = String(timestamp);
    const strNonce = String(nonce);

    const tmpStr = [strToken, strTimestamp, strNonce].sort().join('');
...
router.post('/wechat', express.text({ type: 'application/xml', limit: '10mb' }), async (req, res) => {
  try {
    ...
    if (signature && timestamp && nonce) {
      // Ensure params are strings to prevent type juggling
      const strToken = String(webhookToken);
      const strTimestamp = String(timestamp);
      const strNonce = String(nonce);
