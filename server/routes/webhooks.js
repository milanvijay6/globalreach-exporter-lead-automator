...
router.post('/whatsapp', async (req, res) => {
  try {
    ...
    const sentinelAppSecret = await Config.get('whatsappAppSecret');
    const sentinelSignature = req.headers['x-hub-signature-256'];

    if (sentinelAppSecret) {
      if (!sentinelSignature) {
        logger.warn('WhatsApp webhook missing signature');
        return res.sendStatus(401);
      }

      const signatureHash = sentinelSignature.split('sha256=')[1];
      const expectedHash = crypto
        .createHmac('sha256', sentinelAppSecret)
...
router.get('/wechat', async (req, res) => {
  try {
    ...
    if (!signature || !timestamp || !nonce || !echostr ||
        ...
        typeof nonce !== 'string' || typeof echostr !== 'string') {
  ...
  try {
    ...
    if (signature && timestamp && nonce) {
      if (typeof signature !== 'string' || typeof timestamp !== 'string' || typeof nonce !== 'string') {
        logger.warn('WeChat webhook: Invalid parameter types');
        return res.sendStatus(400);
      }
