...
describe('Auth Middleware Security Tests', () => {
  ...
  it('requireAuth should deny access if only req.userId is present (must have req.user object)', () => {
    req.userId = 'user123';

    authMiddleware.requireAuth(req, res, next);

    assert.strictEqual(req.nextCalled, false, 'Should NOT call next() without req.user object');
    assert.strictEqual(res.statusCode, 401, 'Should return 401 status');
  });
});
